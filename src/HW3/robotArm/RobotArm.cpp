...
int RobotControl = 0;
int TeapotControl = 0;	// teapot control - 0일 땐 teapot 안움직임, 1이면 움직임
int RightButtonDown = 0;	// MOUSE
...
float lastX_c = SCR_WIDTH / 2.0f;
float lastY_c = SCR_HEIGHT / 2.0f;
...
glm::mat4 objectXform;// = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.1f, 0.0f)), glm::vec3(0.0015f, 0.0015f, 0.0015f)); // teapot object
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset);	// mouse scroll
...
void DrawFingerBase_2(glm::mat4 model);	// 길이가 짧은 FingerBase
...
const float toRadians = 3.141592265f / 180.0f;	// 각도->라디안
...
void myDisplay()
{
	...
	glm::mat4 arm_1 = glm::mat4(1.0f);
	glm::mat4 arm_2 = glm::mat4(1.0f);
	glm::mat4 fb_wrist = glm::mat4(1.0f);
	glm::mat4 fb_1 = glm::mat4(1.0f);	// left FingerBase 
	glm::mat4 ft_1 = glm::mat4(1.0f);	// left FingerTip
	glm::mat4 fb_2 = glm::mat4(1.0f);	// right FingerBase
	glm::mat4 ft_2 = glm::mat4(1.0f);	// right FingerTip

	base = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	base = glm::rotate(base, BaseSpin * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(base);

	arm_1 = glm::translate(base, glm::vec3(0.0f, 0.4f, 0.0f));
	arm_1 = glm::rotate(arm_1, ShoulderAng * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(arm_1);

	arm_2 = glm::translate(arm_1, glm::vec3(0.0f, 0.5f, 0.0f));
	arm_2 = glm::rotate(arm_2, ElbowAng * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(arm_2);

	fb_wrist = glm::translate(arm_2, glm::vec3(0.0f, 0.5f, 0.0f));
	fb_wrist = glm::rotate(fb_wrist, WristAng * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
	fb_wrist = glm::rotate(fb_wrist, WristTwistAng * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
	DrawFingerBase_2(fb_wrist);

	fb_1 = glm::translate(fb_wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	fb_1 = glm::rotate(fb_1, FingerAng1 * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(fb_1);

	ft_1 = glm::translate(fb_1, glm::vec3(0.0f, 0.35f, 0.0f));
	ft_1 = glm::rotate(ft_1, FingerAng2 * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(ft_1);
	...
	ft_2 = glm::translate(fb_1, glm::vec3(0.0f, 0.35f, 0.0f));
	ft_2 = glm::rotate(ft_2, -FingerAng2 * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(ft_2);

	

	
	objectXform = glm::translate(model, glm::vec3(TBaseTransX, 0.0f, TBaseTransZ));
	
	objectXform = glm::translate(base, glm::vec3(BaseTransX - TBaseTransX, 0.0f, BaseTransZ - TBaseTransZ));
	objectXform = glm::rotate(objectXform, TBaseSpin * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
	objectXform = glm::translate(base, glm::vec3(-BaseTransX + TBaseTransX, 0.0f, -BaseTransZ + TBaseTransZ));
	
	objectXform = glm::rotate(objectXform, TShoulderAng * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::rotate(objectXform, TElbowAng * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
	
	objectXform = glm::rotate(objectXform, TWristAng * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::rotate(objectXform, TWristTwistAng * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));

	

	//objectXform = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f)), glm::vec3(0.08f, 0.08f, 0.08f));
	objectXform = glm::translate(objectXform, glm::vec3(0.5f, 0.1f, 0.0f));
	objectXform = glm::scale(objectXform, glm::vec3(0.08f, 0.08f, 0.08f));	//rabbit: objectXform = glm::scale(objectXform, glm::vec3(0.08f, 0.08f, 0.08f));
	DrawObject(objectXform);
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// space bar
		...
		TeapotControl = (TeapotControl + 1) % 2;	// 값이 0일 땐 1로, 1일 땐 0으로 변경
	...
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
		camera.ProcessKeyboard(LEFT, deltaTime*!0);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	float xoffset_c = xpos - lastX_c;
	float yoffset_c = lastY_c - ypos; // reversed since y-coordinates go from bottom to top
	...
	if (LeftButtonDown)
	{
		...
		if (TeapotControl) {
			switch (RobotControl)
			{
			case 0: TBaseTransX += xoffset; TBaseTransZ -= yoffset; break;
			case 1: TBaseSpin += xoffset * 180; break;
			case 2: TShoulderAng += yoffset * -90; TElbowAng += xoffset * 90; break;
			case 3: TWristAng += yoffset * -180; TWristTwistAng += xoffset * 180; break;
			}
		}
	} 
...
// 길이가 짧은 FingerBase
void DrawFingerBase_2(glm::mat4 model)
{
	glm::mat4 Base = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.1f, 0.0f));
	Base = glm::scale(Base, glm::vec3(0.06f, 0.15f, 0.06f));
	glm::mat4 InBase = glm::inverse(Base);
