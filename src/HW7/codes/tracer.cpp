...
int tracer::rayHalfspaceIntersect(ray* r, halfspace* s, float* t) {
	point p;   /* start of transformed ray */
	point* v;
...
void tracer::findHalfspaceNormal(halfspace* s, point* p, vector* n) {
	n->x = 0.0;
	n->y = 0.0;
	n->z = 1.0;
	n->w = 0.0;
...
void tracer::trace(ray* r, point* p, vector* n, material** m) {
	...
	/*hit2 = rayHalfspaceIntersect(r, s2, &t);
	if (hit) {
		p->w = 0.0;
	}
	else {
